 ...
 * performance (otherwise every marker re-snapshots on every render), but
 * that freezes the marker into a static bitmap on its very first render —
 * if the remote avatar <Image> hasn't finished loading by then, it's baked
 * in as permanently blank. This keeps tracksViewChanges true only until the
 * image actually finishes loading (or fails/has none), then freezes it.
 */
